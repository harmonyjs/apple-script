...
export function extractZodIssues(parseResult: { success: false; error?: unknown }): z.ZodIssue[] {
...
export function createParamMap<T extends Record<string, unknown>>(params: unknown): T {
  if (!params || typeof params !== "object") return {} as T;
  return params as T;
}

export function getArrayElement<T>(arr: unknown[], index: number, defaultValue?: T): T | undefined {
...
export function extractErrorCode(error: unknown): number | undefined {
  ...
  if (hasProperty(error, "cause") && hasProperty((error as any).cause, "code")) {
...
export interface NormalizableOperation { normalizeRows?: boolean; output: unknown }
...
export function getNormalizeRowsSetting(def: unknown, defaultValue: boolean): boolean {
  if (hasProperty(def, "normalizeRows")) return Boolean((def as any).normalizeRows);
  return defaultValue;
}

export function safeArraySlice<T>(arr: unknown, start: number, end?: number): T[] {
