...
import type { ProcessingConfig, PipelineContext, BuiltScriptContext, Step } from "./types.js";
import { 
  InputValidationStep, 
  ScriptBuildStep, 
  ProtocolParseStep, 
...
export function createProcessingPipeline(cfg: ProcessingConfig) {
  ...
  return {
    ...
    runPostProcessing<TInput extends z.ZodType, TOutput extends z.ZodType, TValidate extends boolean>(
      def: OperationDef<TInput, TOutput>,
      payload: string,
      ...
      if (!pipeline) {
        throw new Error(`Unknown operation kind: ${def.kind}. Supported kinds: ${Object.keys(pipelineRegistry).join(', ')}`);
      }
...
export function toProcessingConfig(cfg: RunnerConfig & { ensureAppReady?: boolean }): ProcessingConfig {
  return {
    appId: cfg.appId,
