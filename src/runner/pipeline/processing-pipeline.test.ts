...
void test("processing-pipeline", async (t) => {
  ...
  await t.test(
    "createProcessingPipeline returns pipeline with correct interface",
    () => {
  ...
  await t.test(
    "pipeline.validateInputIfNeeded validates input when enabled",
    () => {
      const config = toProcessingConfig(createMockRunnerConfig());
      const pipeline = createProcessingPipeline(config);
      ...
      pipeline.validateInputIfNeeded(
        operation,
        { invalid: "data" } as any,
        false,
      );
  ...
  await t.test(
    "pipeline.buildScriptContext builds complete script context",
    () => {
      const config = toProcessingConfig(createMockRunnerConfig());
      const pipeline = createProcessingPipeline(config);
      const operation = createMockScalarOperation();
  ...
  await t.test(
    "pipeline.runPostProcessing throws error for unknown operation kind",
    () => {
      const config = toProcessingConfig(createMockRunnerConfig());
      const pipeline = createProcessingPipeline(config);
      const operation = {
      ...
      assert.throws(
        ...
        (error: Error) =>
          error.message.includes("Unknown operation kind: unknown"),
      );
    },
  );

  await t.test(
    "pipeline integration: complete flow for different operation types",
    () => {
      const config = toProcessingConfig(createMockRunnerConfig());
      const pipeline = createProcessingPipeline(config);
      ...
      const scalarResult = pipeline.runPostProcessing(
        scalarOp,
        "scalar-result",
        false,
      );
      ...
      const rowsResult = pipeline.runPostProcessing(
        rowsOp,
        "col1\u001Fcol2\u001Erow2col1\u001Frow2col2",
        false,
      );
      ...
      const sectionsContext = pipeline.buildScriptContext(
        sectionsOp,
        sectionsInput,
      );
      assert(typeof sectionsContext.script === "string");
      const sectionsResult = pipeline.runPostProcessing(
        sectionsOp,
