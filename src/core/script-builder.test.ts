...
test("script-builder", async (t) => {
  await t.test("buildAppleScript", async (t) => {
    await t.test("builds basic scalar script", () => {
      ...
      assert.ok(script.includes('tell application id "com.apple.Safari" to launch'));
    ...
    await t.test("builds script with parameters", () => {
      const params = [
        { varName: "__ARG__url", literal: '"https://example.com"', paramName: "url" },
      ...
      assert.ok(script.includes('return __ARG__url & " has " & __ARG__count & " tabs"'));
    });
    ...
    await t.test("builds action script", () => {
      ...
      assert.ok(script.includes(`if __code__ is not in {"${ACTION_CODES.FAILURE}", "${ACTION_CODES.SUCCESS}", "${ACTION_CODES.PARTIAL}"}`));
    ...
    await t.test("builds rows script", () => {
      const script = buildAppleScript({
        appId: "com.apple.Safari",
        kind: "rows",
        userScript: 'return {{"row1col1", "row1col2"}, {"row2col1", "row2col2"}}',
        params: [],
        timeoutSec: 10,
        ensureReady: true,
      });
      ...
      assert.ok(script.includes("if class of __rows__ is not list then error -10002"));
    ...
    await t.test("builds sections script", () => {
      const script = buildAppleScript({
        appId: "com.apple.Safari",
        kind: "sections",
        userScript: 'return {{"section1", {"item1", "item2"}}, {"section2", {"item3"}}}',
        params: [],
        timeoutSec: 10,
        ensureReady: true,
      });
      ...
      assert.ok(script.includes("if class of __sections__ is not list then error -10003"));
      ...
      assert.ok(script.includes(`"${US}" & my __join__(__text_items__, "${US}")`));
      assert.ok(script.includes(`my __join__(__encoded__, "${GS}")`));
    });

    await t.test("handles complex parameter substitution", () => {
      const params = [
        { varName: "__ARG__message", literal: '"Say " & quote & "Hello" & quote & ""', paramName: "message" },
        { varName: "__ARG__items", literal: '{"a", "b", "c"}', paramName: "items" },
      ];

      const script = buildAppleScript({
        appId: "com.apple.Safari",
        kind: "scalar",
        userScript: "set msg to ${message}\nset list to ${items}\nreturn msg & (count of list)",
        params,
        timeoutSec: 10,
        ensureReady: true,
      });
      ...
      // Should contain parameter declarations
      assert.ok(script.includes('set __ARG__message to "Say " & quote & "Hello" & quote & ""'));
    ...
    await t.test("handles special characters in app ID", () => {
      ...
      assert.ok(script.includes('tell application id "com.company.app-name_v2"'));
      assert.ok(script.includes('tell application id "com.company.app-name_v2" to launch'));
    });
  });
  ...
  await t.test("error handling in generated scripts", async (t) => {
    await t.test("scalar script handles missing return", () => {
      ...
      assert.ok(script.includes("if __val__ is missing value then error -10001"));
    });
    ...
    await t.test("all scripts have error handling wrapper", () => {
      ...
      for (const kind of kinds) {
        ...
        assert.ok(script.includes(`return "ERR" & "${GS}" & errNum & "${GS}" & (errMsg as text)`));
      }
    });
  });
...
if myVar contains "special" then
  ...
  return "nothing"
      ...
      assert.ok(script.includes('      set myVar to "special chars: \\"quotes\\" & tab & return"'));
  ...
  await t.test("edge cases", async (t) => {
    await t.test("handles parameter names with special characters", () => {
      const params = [
        { varName: "__ARG__param_with_underscores", literal: '"value"', paramName: "param_with_underscores" },
        { varName: "__ARG__param123", literal: '"value"', paramName: "param123" },
      ];
      ...
      assert.ok(script.includes("return __ARG__param_with_underscores & __ARG__param123"));
    });
